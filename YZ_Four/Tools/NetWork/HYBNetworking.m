...
//多张图片
+ (HYBURLSessionTask *)pictures_uploadWithDic:(NSMutableArray *)mutArray
                                   url:(NSString *)url
                              mimeType:(NSString *)mimeType
                            parameters:(NSDictionary *)parameters
                              progress:(HYBUploadProgress)progress
    ...
    HYBURLSessionTask *session = [manager POST:url parameters:parameters constructingBodyWithBlock:^(id<AFMultipartFormData>  _Nonnull formData) {
        ...
        for (NSDictionary *tempDic in mutArray) {
            ...
            NSString *name = tempDic[@"fileName"];

            NSString *imageFileName = @""; // 使用时间命名

            if (kStringIsEmpty(imageFileName)) {
                             ...
                             fail:(HYBResponseFail)fail
{
    ...
    [manager DELETE:absolute parameters:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable responseObject) {
